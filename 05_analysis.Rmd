...
ggplot(ZC_pivot, aes(x = UTC, y = value, color = avg)) + geom_smooth() + 
  ...
  labs(title = "Rolling Averages of ZC Detections alongside Soundscape Metrics - Buoy 23")
```
# Using rollApply for PM
```{r}
PM_filt <- GPSwhaleAISenvSS23 %>% 
...
ggplot(PM_pivot, aes(x = UTC, y = value, color = avg)) + geom_smooth() + 
  ...
  scale_x_datetime(date_breaks = "3 days") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  labs(title = "Rolling Averages of PM Detections alongside Soundscape Metrics - Buoy 23")
```
...
```{r}
ggplot(allDrifts, aes(factor(Wpresence), TOL_125)) + geom_boxplot()
```
