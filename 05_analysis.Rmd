...
```{r loudest-buoy}
allDrifts %>%
  ggplot(mapping = aes(x = Deployment, y = TOL_2000)) + geom_boxplot() + theme_bw()
...
#Plot facet grid
ggplot(ZC_pivot, aes(x = UTC, y = value, color = avg)) + geom_smooth() +
  facet_grid(vars(avg), scales = "free") +
  scale_x_datetime(date_breaks = "3 days") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  labs(title = "Rolling Hourly Averages of ZC Detections and Soundscape Metrics -  
       Buoy 21")
```
# rollapply for vessels
```{r}
ZC_filt_ship <- GPSwhaleAISenvSS08 %>% 
  filter(species %in% c("ZC", NA)) %>%
  # Hourly rolling averages
  mutate(rollAvg_Whl = zoo::rollapply(Wpresence, 5, mean, fill = NA)) %>%
  mutate(rollAvg_2000 = zoo::rollapply(TOL_2000, 5, mean, fill = NA)) %>%
...
```{r}
ZC_filt_ship <- GPSwhaleAISenvSS23 %>% 
  filter(species %in% c("ZC", NA)) %>%
  # Hourly rolling averages
  mutate(rollAvg_Whl = zoo::rollapply(Wpresence, 5, mean, fill = NA)) %>%
  mutate(rollAvg_2000 = zoo::rollapply(TOL_2000, 5, mean, fill = NA)) %>%
...
min_first <- min(ZC_filt$rollAvg_Whl)
max_first <- max(ZC_filt$rollAvg_Whl)
min_second <- min(ZC_filt$rollAvg_2000)
max_second <- max(ZC_filt$rollAvg_2000)
...
ggplot(ZC_filt, aes(x = UTC)) +
  ...
  geom_smooth(aes(y = inv_scale_function(rollAvg_2000, scale, shift), color = "TOL 2000")) +
  scale_x_datetime(date_breaks = "3 days") +
